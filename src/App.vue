<template>
  <div id="app"  class="app-wrapper">
    <div class="player-wrapper">
      <div class="container-background" :style="{'background-image': 'url(' + musicImage + ')'}">
      </div>
      ...
      <div class="container-player">
        <div :class="'container-music-info ' + playerStatus">
          ...
          <div class="container-music-time">
            <div class="container-music-current-time">{{musicInfo.current}}</div>
            <div class="container-music-duration">{{musicInfo.durationFormat}}</div>
          </div>
          <div class="container-music-duration-flow" @mousemove="showInstantTime" @mouseout="hideInstantTime" @click="setProgress">
            <div class="container-music-instant-time" :style="instantTimeStyle">{{instantTimeFormat}}</div>
            <div class="container-control-hover" :style="{width:hoverWidth}"></div>
            <div class="container-music-active-bar" :style="{width:activeLength}"></div>
          </div>
        </div>
        <div class="container-player-main">
          <div :class="'container-album ' + playerStatus">
            <img :src="musicImage" alt="歌曲图片" :class="'container-music-image ' + playerStatus">
            <div class="container-author">夜琉璃</div>
          </div>
          <div class="container-controls">
            <i class="control-btn fa fa-backward" @click="playControl"></i>
            <i :class="'control-btn fa ' + playIcon" @click="playControl('play')"></i>
...
export default {
  ...
  data(){
    return {
      musicImage:"https://p2.music.126.net/iAaRt_l_ussYqkLVuDEpqg==/2931298001578529.jpg",
      musicInfo:{
        url:"",
        name:"走在冷风中",
        artist:"周二珂",
  ...
  methods:{
    ...
    fetchMusic:function () {
      // mid 歌单ID
      fetch("https://api.uomg.com/api/rand.music?mid=868773387&format=json")
        .then((res)=>{
          return res.text()
        }).then((res)=>{
        ...
        this.audio.onloadedmetadata = ()=> {
          this.musicInfo.durationFormat = this.timeFormat(this.audio.duration);
    ...
    showInstantTime:function (e) {
      ...
      this.musicProgress = this.musicInfo.duration * (e.layerX / e.target.offsetWidth);
      this.instantTimeFormat = this.timeFormat(this.musicProgress);
      this.instantTimeStyle = "left:" + e.layerX + "px" + ";display:block;margin-left:-21px"
    },
...
#app {
  ...
  @keyframes rotateAlbum {
    ...
    .container-music-current-time ,.container-music-duration{
      color: #f86d92;
      font-size: 11px;
